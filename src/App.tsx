            ...
            const renderCard = (card, zone = "battlefield") => {
              ...
              return (
                <div key={card.instanceId}
                  ...
                  {/* Ability icons — strip shown above card, below header */}
                  {(card.abilities || []).length > 0 && (
                    <div style={{ position: "absolute", top: -18, left: 0, right: 0, display: "flex", gap: 2, justifyContent: "center", flexWrap: "nowrap", zIndex: 20, pointerEvents: "none" }}>
                      {(card.abilities || []).slice(0, 5).map(key => {
                        const ab = ABILITIES.find(a => a.key === key);
                        return ab ? (
                          <span key={key} title={ab.name} style={{ fontSize: isMe ? 12 : 9, lineHeight: 1, background: "#000000ee", borderRadius: 3, padding: "1px 3px", border: `1px solid ${ab.text}55` }}>
                            {ab.icon}
                          </span>
                        ) : null;
                      })}
