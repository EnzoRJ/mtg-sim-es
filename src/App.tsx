        ...
        {(() => {
          ...
          const renderCard = (card, zone = "battlefield") => {
            ...
            <div key={card.instanceId}
              ...
              onDrop={e => { e.preventDefault(); e.stopPropagation(); if (dragCard && isMe && dragCard.zone === zone) { reorderZone(pid, zone, dragCard.instanceId, card.instanceId); } setDragCard(null); setDragOverId(null); }}
              ...
              <div style={{ flex:1, display:"flex", flexDirection:"row", minHeight:0, overflow:"hidden", borderBottom: lands.length > 0 ? "1px solid #1a1a2e" : "none" }}>
                ...
                <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0, minHeight:0 }}>
                  ...
                  <div
                    ...
                    onDragOver={e => { e.preventDefault(); e.currentTarget.style.background="#1a1a2e22"; }}
                    onDragLeave={e => { e.currentTarget.style.background=""; }}
                    onDrop={e => { e.preventDefault(); e.currentTarget.style.background=""; if (dragCard && isMe) setRow2Cards(s => { const n=new Set(s); n.delete(dragCard.instanceId); return n; }); setDragCard(null); setDragOverId(null); }}
                  ...
                  {(isMe || permanents.some(c => row2Cards.has(c.instanceId))) && (
                    <div
                      onDragOver={e => e.preventDefault()}
                      onDrop={e => { e.preventDefault(); if (dragCard && isMe) setRow2Cards(s => new Set([...s, dragCard.instanceId])); setDragCard(null); setDragOverId(null); }}
                      style={{ overflowX:"auto", overflowY:"hidden", padding:"4px 8px", display:"flex", gap:5, alignItems:"flex-start", flexWrap:"nowrap", minHeight:isMe ? 85 : 0, borderTop:"1px dashed #1a1a2e", background: isMe ? "#04040a" : "transparent" }}>
                      {permanents.filter(c => row2Cards.has(c.instanceId)).map(c => renderCard(c))}
                ...
                {/* RIGHT: LOG — fixed width column, never overlaps scroll */}
                {isMe && (
                  <div style={{ width:170, flexShrink:0, borderLeft:"1px solid #1a1a2e", background:"#07070fee", display:"flex", flexDirection:"column", overflowY:"auto" }}>
