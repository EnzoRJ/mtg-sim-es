...
function CardVersionModal({ cardName, onSelect, onClose }) {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        // Fetch Spanish printings first, then English as fallback
        const esQuery = `!"${encodeURIComponent(cardName)}" lang:es`;
        const enQuery = `!"${encodeURIComponent(cardName)}"`;

        const [esRes, enRes] = await Promise.all([
          fetch(`https://api.scryfall.com/cards/search?q=${encodeURIComponent(esQuery)}&unique=prints&order=released&dir=desc`),
          fetch(`https://api.scryfall.com/cards/search?q=${encodeURIComponent(enQuery)}&unique=prints&order=released&dir=desc`),
        ]);

        const esData = esRes.ok ? (await esRes.json()).data || [] : [];
        const enData = enRes.ok ? (await enRes.json()).data || [] : [];

        // Merge: for each EN printing, prefer ES version of same set
        const esBySet = Object.fromEntries(esData.map(c => [c.set, c]));
        const merged = enData.map(enCard => {
          const esCard = esBySet[enCard.set];
          if (esCard) {
            // Use ES image but keep EN metadata for compatibility
            return {
              ...enCard,
            ...
            return (
              <div key={v.id}
                ...
                <div style={{ position:"relative" }}>
                  ...
                  {v._hasSpanish && <div style={{ position:"absolute", top:4, right:4, background:"#1a5a1a", color:"#88ff88", fontSize:9, fontWeight:800, padding:"2px 5px", borderRadius:4 }}>ES</div>}
                </div>
                <div style={{ padding:"6px 8px" }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:4, marginTop:2 }}>
                    ...
                    {v._hasSpanish && <span style={{ fontSize:8, color:"#44ff88" }}>🌐 ES</span>}
                  </div>
                </div>
              </div>
            );
        ...
        {(() => {
          ...
          const renderCard = (card, zone = "battlefield", forceSmall = false) => {
            ...
            <div key={card.instanceId}
              ...
              style={{ position: "relative", display:"flex", flexDirection:"column", alignItems:"center", opacity: isDragging ? 0.4 : 1, outline: isOver ? "2px dashed #ffd700" : "none", borderRadius: 6, cursor: isMe ? "grab" : "default", transition:"opacity 0.15s", overflow: "visible" }}
